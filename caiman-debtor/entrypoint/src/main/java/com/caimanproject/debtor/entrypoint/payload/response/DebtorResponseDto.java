...
@Schema(description = "Full representation of a debtor, including identity, notification settings, audit timestamps, and resolved contact list.")
@Builder
public record DebtorResponseDto(
    ...
    @Schema(
        description = "Whether this debtor is active. When false, the debtor is hidden from plan membership assignment but existing memberships, invoices, and payments are not affected.",
        example = OpenApiConstants.Examples.TRUE,
        nullable = false)
    Boolean active,
    ...
    @Schema(
        description = "Resolved list of contact endpoints for this debtor. Empty list when no contacts are registered.",
        nullable = false)
