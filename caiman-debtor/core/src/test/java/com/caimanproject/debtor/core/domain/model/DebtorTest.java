...
class DebtorTest {
    ...
    class ConstructorTestSuit {
        ...
        class TestCreateBuilder {
            ...
            @ParameterizedTest
            @MethodSource("com.caimanproject.debtor.core.domain.model.DebtorTest#build_duplicate_contacts_by_values")
            void should_fail_when_contacts_have_duplicate_value(
                    final List<DebtorContact> contacts, final List<String> repeatedContactValue) {
                // Given
            ...
            @ParameterizedTest
            @MethodSource("com.caimanproject.debtor.core.domain.model.DebtorTest#build_duplicate_contacts_by_priority")
            void should_fail_when_contacts_have_duplicate_priority(
                    final List<DebtorContact> contacts, final List<String> repeatedContactValue) {
                // Given
                final var debtorBuilder = Debtor.createBuilder()
                        .name("John Doe")
                ...
                abstractThrowableAssert.isInstanceOfSatisfying(DomainException.class, exception -> {
                    final var details = exception.getErrors().stream()
                            .peek(error -> Assertions.assertThat(error.getCode().getFullCode())
                                    .isEqualTo(DomainExceptionCode.DUPLICATE_CONTACT_PRIORITY.getFullCode()))
                            .map(error -> error.getDetail().orElseThrow())
                            .toList();
                    repeatedContactValue.forEach(
                            value -> Assertions.assertThat(details).anyMatch(detail -> detail.contains(value)));
                });
            }
