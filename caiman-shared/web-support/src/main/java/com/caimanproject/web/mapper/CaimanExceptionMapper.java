...
import com.caimanproject.web.config.GlobalRestExceptionHandlerConfig;
...
public class CaimanExceptionMapper {
    ...
    private static ProblemDetailPropertySourceResponseDto buildPropertySourceResponse(final ValidationErrorSource source) {
        final String invalidValue = source.invalidValue();
        return switch (source) {
            case ValidationErrorSourceBody body -> new ProblemDetailPropertySourceBodyResponseDto(invalidValue, body.body());
            case ValidationErrorSourceParameter parameter -> new ProblemDetailPropertySourceParameterResponseDto(invalidValue, parameter.queryParam());
            case ValidationErrorSourceHeader header -> new ProblemDetailPropertySourceHeaderResponseDto(invalidValue, header.header());
            case ValidationErrorSourcePathParameter pathParameter -> new ProblemDetailPropertySourcePathParameterResponseDto(invalidValue, pathParameter.pathParameter());
            case ValidationErrorSourceGeneric generic -> new ProblemDetailPropertySourceGenericResponseDto(invalidValue, generic.fieldName());
        };
    }
}
