 ...
 *
 * <p>Adding, renaming, or removing a nested record requires a matching update in
 * {@code CaimanRuntimeHints#registerHints}. Without it, Hibernate Validator's {@code JPATraversableResolver} will throw
 * {@code MissingReflectionRegistrationError} at native-image runtime when it accesses configuration property fields via
 * reflection.
 */
@Validated
...
public record CaimanServerPropsConfig(
    ...
    public record DatabasePropImpl(
            @NotNull DatabaseType type, String url, String username, String password, String sqliteFile)
        ...
        @AssertTrue(
                message =
                        "DATABASE_TYPE=POSTGRES requires DATABASE_JDBC_URL, DATABASE_USERNAME, DATABASE_PASSWORD environment variables")
    ...
    public record ProjectPropImpl(
            ...
            @NotBlank String description) implements CaimanServerProps.ProjectProp {}
    ...
    public record OpenApiGenericPropImpl(
            @NotBlank String path, @NotNull Boolean enabled) implements CaimanServerProps.OpenApiGenericProp {}
    ...
    public record OpenApiApplicationContactPropImpl(
            ...
            @NotBlank @Email String email) implements CaimanServerProps.OpenApiApplicationContactProp {}
